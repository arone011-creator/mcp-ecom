...
46. **The entire order read path was broken, and the break was masking a cross-user data leak.** `getOrders` and `getOrder` were wrapped in `createCachedFunction`, but both resolve the caller by calling `hasPermission` → `getCurrentUser` → `getServerSession` → `headers()`. Next.js refuses that: *"Route /orders used headers inside a function cached with unstable_cache"*. So `/orders` and `/orders/[id]` returned 500 for every request — the order read path, which is exactly what an MCP `get_order_status` tool needs, did not work at all. The deeper problem is why it must not be cached: `createCachedFunction` passes `undefined` as `unstable_cache`'s `keyParts`, so the cache key derives from the function's **arguments** alone. `getOrders(page, limit, status)` and `getOrder(orderId)` take no user — they read identity from the session inside the closure. A working cache would therefore have keyed one user's orders under an argument list any other user reproduces, and served user A's orders to user B. Next's error was the only thing preventing that. Both functions are now uncached, with a comment saying why, so nobody re-adds it.
47. **`app/admin/orders/page.tsx` called `getOrders` with the wrong shape and typechecked anyway.** It passed `{ page, limit, search, status, dateFrom, dateTo }` to a function whose signature is `(page, limit, status)`. It compiled because `createCachedFunction`'s generic erased the parameter types; removing the wrapper in finding 46 surfaced it immediately as a type error. In production `page` received the whole object and the search and date filters were silently discarded. Call site corrected to positional arguments. The filters are still not implemented — admin polish is out of scope by the user's decision — but they are now visibly unimplemented rather than silently ignored.
48. **A tooling artifact worth not mistaking for an app bug.** Synthetic coordinate clicks from the browser tool did not reach React's synthetic event handlers, because the Browser pane was not displayed and therefore not compositing frames (the same reason `screenshot` failed). The button was present, enabled, hit-testable at the click point, and React was demonstrably hydrated — `__reactProps` were attached. Invoking the handler directly through `__reactProps.onClick` signed in immediately. Nothing was wrong with the sign-in form; the rest of the flow was driven by invoking React handlers directly.
49. **Task 13's own instructions told me to set `NODE_ENV=production` on Railway.** That is finding 21, the mistake that cost ~16 builds and six wrong hypotheses during Task 0, written back into the plan by me. Next sets `NODE_ENV` correctly per command and Railway sets it for the runtime; setting it by hand is what broke the build before. Removed from the plan. Worth noting that recording a lesson in a findings list did not stop me reproducing it in a procedure a few hundred lines further down the same document.
50. **`.env.production` was committed to the repository, and it is a booby trap on a public deploy.** It is a docker-compose leftover containing placeholders, including `NEXTAUTH_SECRET="your-nextauth-secret-here-change-in-production"`. Next loads `.env.production` during a production build. Real environment variables take precedence, so correctly-set Railway values win — but any variable that is ever missing silently falls back to this file, and the fallback for the session signing secret is a value published in a public GitHub repository. Anyone could then forge session JWTs, with nothing in the logs to indicate it. Deleted from git and added to `.gitignore`; `docker-compose.yml` will now fail loudly if the file is absent, which is the correct failure mode. Found by scanning tracked files for credential patterns before the first push to a public repo.
51. **`build.durationMs` was gated on a metric too noisy to gate.** The final M1 gate failed on `build.durationMs: 195258 -> 220378`, a 12.9% rise against a 10% tolerance. Measuring the same commit three times gave **195s, 220s and 255s** — a 30% spread, with the *slowest* run on an otherwise idle machine. `prisma generate`, newly added to the build in Task 11, accounts for about 22s of it; the rest is machine load. A metric whose variance exceeds its own tolerance can only generate false alarms, and the tempting response — re-running until it goes green — is cherry-picking. Duration is now recorded and not gated. `standaloneBytes` remains gated: it is deterministic, and it is the number that actually matters for a deploy.
52. **The deploy was verified against a stale container once.** The first live probe run reported 500s on `/products/[slug]`, `/category/[slug]`, `/api/auth/providers` and `/checkout`. Those were real responses, but from the *previous* deployment — the one built before the environment variables existed; `get-status` showed the new deployment still `BUILDING`. Every one of them cleared on the new container. A green `HTTP 200` on `/` is not evidence that the deployment you think you are testing is the one answering.
53. **The header was an 11-line stub with no navigation at all.** `components/header.tsx` rendered a single `<h1>E-commerce Store</h1>` and nothing else — no cart link, no sign-in, no nav. A visitor could add items to the cart and then had no way to reach it except by typing `/cart` into the address bar. Reported by the user from a screenshot, not caught by me: every check I ran hit routes directly by URL, so I never once navigated the site the way a person would. `components/cart-drawer.tsx` existed, read from the server correctly, and was imported by nothing. Replaced with a server-component header carrying nav, a search box, a cart link with a live item count, and sign-in/account state.
54. **`/products` was the most-linked route in the app and it 404'd.** Eleven components linked to it: the home page's primary call to action, the breadcrumb on every product and category page, "Continue Shopping" in the cart, the empty state on search, and the cart drawer. I deferred this as low-value on the user's steer that low-value bugs were acceptable — that judgement was wrong, and the link count was one grep away the whole time. "Low value" has to be measured by how much of the app points at something, not by how the feature sounds.
55. **The second hero button was invisible and pointed nowhere.** `app/page.tsx` styled it `variant="outline"` (which paints a light background) plus `text-white`, giving white text on white, and linked it to `/category/featured` — a slug that was never seeded, so it would have 404'd had anyone been able to read it. It rendered as a blank white rectangle. Now `bg-transparent`, labelled "Search products", pointing at `/search`.

**What findings 53–55 have in common:** all three are things a person sees in the first five seconds and I did not, because I verified with `curl` against known URLs and with a browser I drove straight to the page I wanted to test. Route-level probing proves a route answers; it does not prove the route is reachable.
56. **The app and its database were on opposite sides of the Pacific.** Railway deployed to `sfo` (San Francisco); Supabase sits in `ap-southeast-1` (Singapore). Every query crossed the Pacific, and for a user in India the response crossed it again — India → SFO → Singapore → SFO → India. Measured server-side, `/cart` p50 is **18ms**, so the application itself was never slow; TTFB from India was ~390ms for *every* route including `/robots.txt`, which touches no database and no session. That equality is the proof: the cost was distance, not code. One bucket showed p50 of 7,026ms immediately after a deploy — a cold boot, not steady state. Railway's API does not expose region changes, so the move to Singapore was done by the user in the dashboard.
57. **The header did a database write on every page view.** `getCart()` routes through `getCartSession()`, which performs a `cart.upsert` before reading line items — so rendering the cart badge cost a write plus a read on every page, and would have created a cart row for any visitor who merely loaded a page. It did not, only because `cookies().set()` throws during a Server Component render and both `getCart()` and my header swallowed the error; the badge was correct by accident. Replaced with a read-only `getCartItemCount()`: one aggregate, creates nothing, sets no cookie, returns 0 when no cart cookie exists. A dead `getCartItemCount(userId)` that loaded every cart row to sum quantities, and threw on failure, was removed at the same time — nothing referenced it.
58. **`connection_limit=1` serializes every database operation in the application.** It came from the `.env.local` template written in Task 2 and was copied verbatim into Railway. Invisible with one user; with any concurrency every request queues behind every other request's round trip. Flagged to the user rather than changed, since the value lives in a secret this session deliberately never handles. **Resolved** — raised to 10 by the user. Verified behaviourally rather than by reading the value back (Railway redacts it): against a no-database control, 12 parallel `/cart` requests finished 76ms behind 12 parallel `/robots.txt`, and doubling to 24 parallel *lowered* per-request time from 65ms to 47ms. A pool of one does the opposite — per-request time rises linearly as requests queue. Server-side p95 under that load was 45–159ms, against 4,314–9,614ms before the region move and this change.
59. **`getToken` reads the session cookie *before* the `Authorization` header** (`node_modules/next-auth/jwt/index.js:82-86`), so a cookie silently wins over an explicitly presented bearer token. `requireApiUser` inverts this deliberately: an explicit credential beats an ambient one, and a bearer token that fails to decode is a hard refusal rather than a silent downgrade to whatever cookie rode along on the request. Bearer support itself is native — next-auth v4 already falls back to the header — but the precedence and the failure mode are ours.
60. **`searchProducts` returns `{ products, pagination: { page, limit, total, pages } }`, not `{ products, total }`.** The fixtures in this plan's Task 16 and Task 18 asserted the wrong shape, which would have produced routes that passed their tests and returned `undefined` pagination. Task 16 Step 1's instruction to read the real signature first is what caught it — the plan's own summary of the function was wrong.
61. **`searchProducts` and `getProductById` both select whole `Product` rows, which include `costPrice`.** The storefront gets away with it because it renders server-side and never ships the row; an API hands the row straight over, publishing the margin on every item. Fixed with an allowlist projection (`app/api/v1/_lib/product-view.ts`) rather than a denylist, so a column added to the schema later stays private until someone publishes it deliberately. `trackQuantity` and `barcode` are withheld for the same reason.
62. **`getProductById` has no status filter.** The storefront only ever reaches it from links to published products, but an API can be handed any id, making unreleased and archived products readable by anyone who guesses one. The v1 detail route 404s anything not `PUBLISHED`, reporting it exactly as an absent product so it cannot be used to probe for what is coming.
63. **`searchProducts` ignores `sortOrder` when called in its object form.** `sortOrder` is destructured with a `'desc'` default, but the object branch never sets it, so `sort=price` is always descending and ascending price sort is unreachable. Not fixed — it needs a signature change, and nothing in M2 depends on it. Recorded for whoever adds "cheapest first".
64. **`cancelOrder` lived in a `'use server'` module, where every export is reachable over HTTP with client-supplied arguments.** This plan's Task 17 assumed the route could delegate to it, but `cancelOrder` resolves identity through `getCurrentUser()` → `getServerSession()` → cookies, which a bearer-token caller does not have; the route would have returned 401 to a properly authenticated agent. The obvious repair — adding an `actingUserId` parameter — would have been considerably worse: a server action taking a trusted user id from its caller is an open invitation to cancel anyone's order. The rules moved to `server/orders/cancel-order.ts`, which is *not* a server action and takes its actor explicitly; the action resolves identity from the cookie, the route from the verified token, and both delegate. The ten M1 tests for `cancelOrder` passing unchanged is what made the move safe.
65. **`Order.userId` is nullable**, so `where: { userId: undefined }` is not an impossible filter — to Prisma it means *no filter*, and the route returns every order in the system. This is the same shape as M1's cross-user leak arriving by a different route. All three order routes refuse a blank id before constructing a query, with tests that assert the database is never touched.
66. **This plan's rating filter (Task 18 Step 4) filtered after pagination.** It fetched a page, then dropped the low-rated entries from it in JavaScript — which leaves `pagination.total` counting products the caller never sees and returns short or empty pages while matching products sit unexamined on later ones. "Find me 4+ star shoes" would have returned an arbitrary subset and looked like it worked. Implemented in the query instead, resolving qualifying ids with a grouped `having` clause and folding them into the `where`, so the page and the count see the same filter.
67. **The production HTTPS upgrade in `middleware.ts` dropped the query string.** The redirect target was rebuilt from `request.nextUrl.pathname` alone, discarding `search`. Railway terminates TLS and sets `x-forwarded-proto: https`, so the branch never fires there — which is exactly why it went unnoticed. An MCP client configured with an `http://` base URL would have had its filters silently discarded and been handed a cheerful 200 full of the wrong results. Found by running the production build locally, where the branch does fire.
68. **The cancel route confirmed order ids that the read route deliberately hid.** `GET /api/v1/orders/[id]` answers 404 for another user's order specifically so it cannot be used to enumerate ids; `POST .../cancel` answered 403 for the same order, which confirms the id is real. Closing that door on one route while leaving it open on another against the same ids is no closure at all. Both the status and the message now match. Found by live cross-user testing, not by the unit tests — which asserted the 403 the plan specified.
69. **`server/actions/cart.ts` increments an existing line; this plan's cart route would have replaced it.** Same user, same cart, different result depending on whether they used the site or the API — and "add two more" would have silently undone whatever they already had. The v1 route increments to match, with an explicit `mode: 'set'` for replacement and a refusal for any other mode. It also mirrors the action's other rules: published products only, and nothing beyond available stock.
70. **The token endpoint's rate limiting is per-instance and in-process.** One replica today, so the budgets mean what they say; scale past one and the effective allowance multiplies by the replica count. Acceptable now, not acceptable silently later.
71. **Nothing in the storefront calls `cancelOrder`.** There is no cancel control anywhere in the UI — the only caller is the new API route. Pre-existing, not introduced here, but it means the action's behaviour has never actually been exercised by a person.
72. **The demo credentials now mint API tokens, not just browser sessions.** `admin@example.com` / `admin123` has been flagged since M1 as weak for a public URL; `POST /api/v1/auth/token` raises the stakes, since anyone can now exchange those credentials for a 7-day bearer token programmatically with no browser involved. The endpoint's own defences (identical rejections, a hash comparison spent even on a miss, per-address and per-account budgets) slow discovery of an *unknown* password; they do nothing about a published one.

**Net effect of findings 56–58:** TTFB from the user's own machine went ~400ms → ~240ms, and server-side p95 went 4,314–9,614ms → 45–159ms. Neither fix touched application code except the header query in finding 57; the rest was two configuration values. Worth remembering before optimising code: measure where the time actually goes first. The single most useful measurement here was that `/robots.txt`, which touches nothing, was exactly as slow as pages that touch everything.
